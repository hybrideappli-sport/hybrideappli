...
2. ~~**Régime d'accès.**~~ **Tranché par le fondateur le 2026-09-06** : **authentifié, hors quota produit, non premium** — à l'image de `POST /schedule/incidents`. La carte ne consomme ni moteur ni donnée de santé, elle n'entre donc pas dans le compteur d'accès libre (ADR-008).

   **Assorti d'un garde-fou technique par utilisateur contre l'emballement de consommation Stadia**, explicitement qualifié par le fondateur de **protection budgétaire et non de quota produit** : il ne doit jamais être présenté à l'utilisateur comme une limite d'usage, ni compter dans un quota commercial.

   ⚠️ **Contrainte structurelle à respecter dans la forme retenue.** Les crédits Stadia sont brûlés par les requêtes de tuiles que **le client MapLibre envoie directement au CDN Stadia** (§4, constat des deux hôtes). Ces requêtes **ne transitent pas par notre serveur**. Un limiteur posé sur `/api/v1/map/trails` protège Overpass et **strictement rien du budget Stadia** — s'en contenter donnerait une fausse impression de protection. Le `FixedWindowRateLimiter` existant, en mémoire de processus, ne peut pas davantage voir ces appels.

   **Forme retenue (défense en profondeur). Le levier principal est côté client — le serveur ne peut ici qu'observer :**
   - **L1 — debounce des mouvements de carte.** *Le levier de premier rang.* Un panoramique ou un zoom continu, non débouncé, déclenche des dizaines de chargements de tuiles pour un seul geste de l'utilisateur. Ne déclencher le rafraîchissement qu'à la **stabilisation** de la vue (`moveend` / `zoomend` débouncés), jamais pendant le geste. C'est la mesure qui supprime le plus de crédits pour le moins de code, et elle améliore aussi la fluidité perçue.
   - **L1 — bornes de zoom.** `maxZoom` (et `maxBounds` le cas échéant) pour supprimer la classe de comportements les plus coûteux : zoom profond répété sur une grande étendue.
   - **L1 — plafond client, dans `transformRequest` de MapLibre.** Le seul point par lequel passent toutes les requêtes de tuiles. Compteur de tuiles chargées sur la durée de vie de la page ; au-delà du plafond, les requêtes de tuiles sont refusées et la carte gèle son fond en affichant un état explicite (« exploration limitée, rechargez la page »), les tracés déjà chargés restant lisibles. Plafond à calibrer à la fin de L1 sur une session de navigation réelle. **Limite honnête : c'est du code client, contournable par un utilisateur déterminé.** Ce n'est pas une faille — la menace visée est l'emballement involontaire, pas l'attaque.
   - **L4 — le filet côté fournisseur, seul réellement contraignant.** Trois points à instruire par `devops` à la souscription (à joindre à la question 5) :
     - **Restriction de la clé de tuiles à nos domaines.** `NEXT_PUBLIC_MAP_TILES_API_KEY` est **publique par construction** — elle est lisible dans le bundle et dans l'inspecteur réseau de n'importe quel visiteur. Non restreinte, elle est utilisable par n'importe qui **à nos frais**, et aucun garde-fou client n'y change quoi que ce soit. C'est le point le plus important de la liste : la restriction par domaine référent doit être **posée dès qu'une clé existe**, y compris sur la clé de développement, et pas seulement en L4.
     - **Plafond de dépense dur et alertes dans le dashboard Stadia** : vérifier ce que la console permet réellement (plafond bloquant ou simple notification), et le configurer. Le palier Starter facture le dépassement à 3 ¢/1 000 crédits *sans plafond documenté* (§1).
     - Si **aucun plafond dur n'existe** chez le fournisseur, c'est un **risque budgétaire résiduel** à accepter explicitement, par écrit, avant l'ouverture au public — pas à découvrir sur une facture.

   *Propriétaires : `developer` (L1), `devops` (L4), fondateur (validation de la forme et du plafond).*
...
6. ~~**Dénivelé.** Accepter « — » durablement, ou conditionner l'ouverture au public au lot L4 ?~~ **Tranché par le fondateur le 2026-09-06** : **le champ dénivelé est retiré de la bottom sheet en L1–L3**, il n'y figure pas du tout. Motif : un champ affichant « — » en permanence se lit comme une donnée manquante, donc comme un bug, et non comme une absence assumée. La bottom sheet de la phase 1 affiche donc **nom, distance, surface** — trois champs, tous renseignés.
