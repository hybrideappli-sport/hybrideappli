...
- **Statut** : Accepté
- **Date** : 2026-08-04 — **révisé le 2026-08-07** (§2, §7 réécrit, §8 ajouté) — **révisé le 2026-08-09** (§9 ajouté : activation des documents de consentement par environnement) — **révisé le 2026-08-11** (§10 ajouté : nouvelle version du texte `health_data_processing`, cohérente avec la conservation des indicateurs de sécurité au retrait de ce consentement)
- **Décideur** : `architect`
...
### 10. Texte du consentement `health_data_processing` : cohérence avec la conservation des indicateurs de sécurité (2026-08-11)

**Le problème.** Le commit `576bbf2` (fix(US-01): interaction B1 × B6) a changé le comportement de `POST /api/v1/consents/:code/revoke` pour `health_data_processing` : les `risk_flags` de type `pathology`/`minor` ne sont **plus** purgés au retrait (ils continuent de porter l'avertissement médical fixe, AC3 — voir `apps/web/lib/orchestration/purge-health-data-on-revoke.ts`) ; seuls `pregnancy`/`eating_disorder_history`/`other` le sont toujours. La version `1.0.0` du texte (§1, `0010_seed_referentials.sql`) promettait pourtant sans réserve : « Le retrait entraîne la purge de ces données ». Une contre-revue indépendante a relevé la contradiction entre le texte acquitté et le comportement réel.

**Décision.** Amender le texte, pas le comportement. Les indicateurs de sécurité doivent survivre au retrait de ce seul consentement — c'est le comportement voulu par `576bbf2` (sans lui, l'utilisateur mineur/à pathologie déclarée pourrait faire disparaître l'avertissement médical par une action normale, tout en gardant l'usage de son plan). Le droit à l'effacement **complet** du compte (`erase_account()`, §8) reste, lui, inchangé et total : c'est la voie déjà existante pour qui veut que rien ne survive, y compris ces indicateurs.

**Mise en œuvre — nouvelle version, jamais une réécriture.** Conformément au principe déjà posé en §1 (« un stockage indéfendable » serait de perdre la correspondance entre une version et le texte exact acquitté) et à l'alternative déjà écartée en §9 (« promouvoir plus tard le texte provisoire par un `UPDATE` casse la correspondance »), la migration `0014_health_data_processing_consent_v1_1_0.sql` **insère** une nouvelle ligne `('health_data_processing', '1.1.0', 'fr', ...)` — la ligne `1.0.0` n'est ni modifiée ni supprimée. Comme `1.0.0` avant elle, `1.1.0` reste un texte non validé juridiquement (« Contenu provisoire » toujours présent dans le corps) : la migration l'insère donc `is_current = false`, et laisse à `supabase/seed.sql` (hors production uniquement, même mécanisme défensif que §9) le soin de l'activer. En production, ni `1.0.0` ni `1.1.0` n'est `is_current` tant que la migration d'activation juridique (question ouverte n°6) n'a pas été publiée — ce correctif ne change rien à ce verrou, il ne fait que corriger le texte qui sera un jour soumis à validation.

**Base légale de la conservation — non tranchée.** Le nouveau texte explique la conservation par un objectif de sécurité, mais s'abstient délibérément de citer un fondement RGPD précis (ex. intérêt légitime, art. 6.1.f) : les indicateurs concernés (pathologie déclarée, statut mineur) sont des données de l'article 9 (catégories particulières), pour lesquelles l'intérêt légitime n'est **pas** au nombre des exceptions de l'art. 9.2 — l'invoquer sans vérification serait une affirmation juridique non fondée, du même ordre que celle que ce correctif corrige. Comme le reste de ce document (§ « Conséquences », « Un DPO / conseil juridique doit valider… »), cette base légale reste une question ouverte pour le conseil juridique, explicitée comme telle dans le texte lui-même plutôt que masquée par une citation d'article approximative.

**Re-consentement : aucun, et c'est cohérent.** `has_active_consent()` (`0001_extensions_enums_helpers.sql`) ne teste que `document_code`, jamais `document_version` — choix déjà documenté dans le commentaire de la fonction (« la bascule vers une nouvelle version de document ne périme volontairement pas le consentement en cours »). Un utilisateur ayant consenti à `1.0.0` garde donc un consentement `health_data_processing` actif après ce changement de version courante, sans reconsentement forcé. C'est le bon choix ici : le comportement système que `1.1.0` documente (conservation des indicateurs de sécurité) s'appliquait déjà, y compris à cet utilisateur, depuis `576bbf2` — la nouvelle version rend visible un comportement déjà en vigueur, elle n'introduit aucun nouveau traitement qui justifierait d'exiger un consentement renouvelé.
