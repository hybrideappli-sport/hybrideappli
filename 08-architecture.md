...
> Phase Plan du cycle Ottho. Rédigé par `architect` le 2026-08-04. **Révisé le 2026-08-07** (arbitrage post-revue du Lot L1 — voir §5 et §12).
...
- `stripe_events.payload` peut contenir des données personnelles (e-mail de facturation) et n'est **pas** couvert par `erase_account()` : rétention et purge à cadrer (§12, question 9).
- Le registre `consents` a besoin d'une durée de conservation explicite (§12, question 8) et d'un job de purge, qui utilisera le même mécanisme de dérogation.
- Le seed de `consent_documents` devient un **prérequis dur** : sans document seedé et marqué `is_current`, aucun consentement ne peut être enregistré (FK composite).
...
|---|---|---|
| `/cron/enqueue-weekly-reviews` | horaire | Enrôle les utilisateurs dont l'heure locale atteint dimanche soir (ADR-011) |
| `/cron/enqueue-objective-checks` | quotidienne | AC14 — dates cibles atteintes |
| `/cron/drain-jobs` | toutes les 5 min | Exécute les lots (`FOR UPDATE SKIP LOCKED`) |
...
4. **Mode dégradé après retrait du consentement santé** : parcours non spécifié par les 14 AC — à cadrer avec `spec-writer`. Depuis le 2026-08-07, le retrait ferme aussi la **modification** des données de santé existantes : l'UI doit expliquer ce blocage plutôt que le subir (ADR-012 §2).
...
8. **Conservation du registre de consentement après suppression de compte** (2026-08-07). L'architecture décide de le conserver, pseudonymisé, comme preuve du consentement recueilli (ADR-010 §8). **Tranché par le fondateur (2026-08-09) : 5 ans** à compter du dernier événement de consentement (voir ADR-010, question ouverte n°4). Le job de purge programmée à cette échéance reste hors périmètre du lot L1 — le registre grossit sans purge automatique tant qu'il n'est pas écrit, ce qui n'affecte pas le schéma actuel.
9. **Rétention de `stripe_events`** (2026-08-07). La table conserve les payloads Stripe bruts, qui contiennent des données personnelles (e-mail de facturation), et n'est pas couverte par `erase_account()` — elle n'a pas de `user_id`. À cadrer : durée de conservation (l'idempotence webhook n'exige que quelques semaines) et job de purge. Non bloquant pour le lot L1, bloquant pour l'ouverture commerciale.
10. **Contenu juridique des 4 `consent_documents` (`medical_disclaimer`, `health_data_processing`, `terms`, `privacy`) provisoire, non validé juridiquement** — ajouté suite à l'audit `code-reviewer` du Lot L1 (finding B3, 2026-08-07/09). Le texte inséré en `0010_seed_referentials.sql` porte explicitement la mention « Contenu provisoire — à faire valider juridiquement avant mise en production » dans son propre corps ; il est inséré avec `is_current = false` (le seed local `supabase/seed.sql` ne l'active pas non plus) pour qu'aucun environnement ne le serve par défaut comme document en vigueur. **Action requise avant mise en production** : (a) faire valider juridiquement les 4 textes (en particulier `health_data_processing`, base légale RGPD art. 9 de tout le traitement de données de santé du produit) ; (b) une fois validés, publier une migration dédiée qui insère la version validée (nouveau `version`, ex. `1.0.1` ou `2.0.0` selon l'ampleur des changements) avec `is_current = true` — ne jamais faire passer `is_current` à `true` sur le contenu provisoire actuel par un simple `UPDATE`. Propriétaire : fondateur (validation juridique) + `architect` (migration d'activation).
