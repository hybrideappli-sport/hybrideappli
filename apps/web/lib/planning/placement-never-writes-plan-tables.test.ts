 ...
 *
 * Durcissement (finding N8, seconde passe `code-reviewer`) : le scan d'imports ci-dessous ne
 * vérifie que le graphe d'imports (`materializePlanVersion`/`regeneratePlan`), pas une écriture
 * DIRECTE sur les tables du moteur — une mutation ajoutant, par exemple,
 * `admin.from("planned_sessions").update({...})` dans `place-plan-version.ts` faisait passer le
 * test à tort. On ajoute donc un second scan, par motif, détectant un appel
 * `.from(<table protégée>).(insert|update|upsert|delete)` sur les tables du moteur
 * (`planned_sessions`, `plan_versions`, `plans`, `plan_weeks`, `plan_blocks`, `decision_traces`).
 * Même dépouillement des commentaires que `hybrid-score-not-in-pipeline.test.ts` (finding N7) pour
 * éviter un faux positif sur de la prose qui nommerait ce motif en exemple.
 */
...
// Tables du moteur, jamais écrites depuis F3 (ADR-016 §2) : F3 ne lit `planned_sessions` qu'en
// `select`, et n'a aucune raison de toucher aux autres tables listées ici.
const ENGINE_WRITE_TABLES = ["planned_sessions", "plan_versions", "plans", "plan_weeks", "plan_blocks", "decision_traces"];
// `.from(<table protégée>).(insert|update|upsert|delete)` — l'écriture DIRECTE, quel que soit le
// chemin d'import emprunté pour obtenir le client (finding N8). Chaînage immédiat (`.from(...).`
// suivi du verbe), comme dans le code réel de `lib/planning/**`.
const FORBIDDEN_WRITE_PATTERN = new RegExp(
  `\\.from\\(\\s*["'\`](?:${ENGINE_WRITE_TABLES.join("|")})["'\`]\\s*\\)\\s*\\.\\s*(?:insert|update|upsert|delete)\\s*\\(`,
);
...
describe("placement-never-writes-plan-tables — F3 ↛ moteur (ADR-016 §2)", () => {
  ...
  it.each(files.map((file) => [path.relative(PLANNING_ROOT, file), file] as const))(
    "%s n'importe ni materializePlanVersion ni regeneratePlan, et n'écrit jamais directement dans une table du moteur",
    (_relative, file) => {
      ...
      expect(
        FORBIDDEN_WRITE_PATTERN.test(content),
        `${file} écrit directement dans une table du moteur (insert/update/upsert/delete sur ${ENGINE_WRITE_TABLES.join(", ")}) — finding N8`,
      ).toBe(false);
    },
  );
});
