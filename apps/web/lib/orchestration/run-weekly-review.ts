...
import { createTraceFactory, diffPlanVersions, evaluateStagnation, generatePlan } from "@hybride/rules-engine";
import type { DecisionTrace, PlanDiffItem, PlanningContext, PlanSnapshot, Ruleset, StoredPlanDiffItem } from "@hybride/domain";
...
async function persistStagnationDiagnosis(
  ...
  try {
    ...
    // (`docs/rulesets/0.1.0-dev.md`) — dégradation explicite, jamais un échec du job complet.
    console.warn(`[weekly-review] evaluateStagnation indisponible pour user=${userId} : ${error instanceof Error ? error.message : error}`);
    return;
  }
  ...
  const { error: engineRunError } = await admin.from("engine_runs").insert({
    ...
    input_snapshot_hash: `weekly-review-stagnation:${userId}:${now}`,
    status: "succeeded",
