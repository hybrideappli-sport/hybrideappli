...
-- Source : 08-architecture.md §5.9 (intitulé canonique du contenu attendu) + plan §3.
--
...
--    jour du 2026-08-06. `is_active = true` uniquement en local/dev (voir `08-architecture.md` §9) :
--    aucun ruleset `0.x` ne peut être activé en production (ADR-007), et la validation Zod qui
--    l'impose sera construite au Lot L2 (`@hybride/domain`), hors périmètre du Lot L1.
...
values (
  ...
  true,
  now(),
  'Ruleset de développement local uniquement (08-architecture.md §9). Garde-fous AC8 validés le 2026-08-06 (docs/rulesets/0.1.0-dev.md). Paramètres restants à trancher avant publication de 1.0.0 en production (ADR-007).'
);
